...
public class DoubleLongMap extends LargeCollection implements   Map<Double,Long>, Serializable{
    public static final long               serialVersionUID = 2l;
    private transient Function<Double, byte[]> keySerFunc       = new DoubleSerDe.DoubleSerFunction();
    private transient Function<Long, byte[]> valSerFunc       = new LongSerDe.LongSerFunction();
    private transient Function<byte[], Double> keyDeSerFunc     = new DoubleSerDe.DoubleDeSerFunction();
    private transient Function<byte[], Long> valDeSerFunc     = new LongSerDe.LongDeSerFunction();
    
    public DoubleLongMap() {
        super();
    }
    
    public DoubleLongMap(String dbName) {
        super(dbName);
    }
    
    public DoubleLongMap(String dbPath, String dbName) {
        super(dbPath, dbName);
    }
    
    public DoubleLongMap(String dbPath, String dbName, int cacheSize) {
    ...
    public void optimize() {
        try {
            ...
            for (Entry<Double, Long> entry : this.entrySet()) {
    ...
    public void putAll(Map<? extends Double, ? extends Long> m) {
        try {
            ...
            int counter = 0;
            for (Map.Entry<? extends Double, ? extends Long> e : m
                    .entrySet()) {
    ...
    @Override
    public Set<java.util.Map.Entry<Double, Long>> entrySet() {
        return new MapEntrySet<Double, Long>(this, this.keyDeSerFunc,
                this.valDeSerFunc);
    }
    ...
    private void readObject(java.io.ObjectInputStream in) throws IOException,
        ...
        valSerFunc = new LongSerDe.LongSerFunction();
        keyDeSerFunc = new DoubleSerDe.DoubleDeSerFunction();
        valDeSerFunc = new LongSerDe.LongDeSerFunction();
        this.deserialize(in);
    }
