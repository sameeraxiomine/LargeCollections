...
public class MapWritableList extends LargeCollection implements List<MapWritable>, Serializable {
    ...
    public MapWritableList() {
        super();
    }
    
    public MapWritableList(String dbName) {
        super(dbName);
    }
    
    public MapWritableList(String dbPath,String dbName) {
        super(dbPath, dbName);
    }
    
    public MapWritableList(String dbPath,String dbName,int cacheSize) {
        super(dbPath, dbName, cacheSize);
    }
    
    public MapWritableList(String dbPath,String dbName,int cacheSize,int bloomFilterSize) {
    ...
    public Iterator<MapWritable> iterator() {
        ...
        return new MapValueIterator<MapWritable>(this.getDB(),this.tDeSerFunc);
    }
    ...
    @Override
    public MapWritable set(int index, MapWritable element) {
        if(index<0 || index>=this.size()){
            throw new IndexOutOfBoundsException();
        }
    ...
    @Override
    public void add(int index, MapWritable element) {
        throw new UnsupportedOperationException();
    }

    @Override
    public MapWritable remove(int index) {
        if(index<0 || index!=(this.size-1)){
            throw new IndexOutOfBoundsException("Can only remove the last element");
        }
        MapWritable e = this.get(index);
    ...
    @Override
    public ListIterator<MapWritable> listIterator() {
        return new MyListIterator<MapWritable>(this,this.tSerFunc,this.tDeSerFunc);
    }

    @Override
    public ListIterator<MapWritable> listIterator(int index) {
        return new MyListIterator<MapWritable>(this,this.tSerFunc,this.tDeSerFunc,index);
    }

    @Override
    public List<MapWritable> subList(int fromIndex, int toIndex) {
        throw new UnsupportedOperationException();
    }
