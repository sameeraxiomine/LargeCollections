...
public class ArrayPrimitiveWritableBytesArrayMap extends LargeCollection implements   Map<ArrayPrimitiveWritable,byte[]>, Serializable{
    ...
    public ArrayPrimitiveWritableBytesArrayMap() {
        super();
    }
    
    public ArrayPrimitiveWritableBytesArrayMap(String dbName) {
        super(dbName);
    }
    
    public ArrayPrimitiveWritableBytesArrayMap(String dbPath, String dbName) {
        super(dbPath, dbName);
    }
    
    public ArrayPrimitiveWritableBytesArrayMap(String dbPath, String dbName, int cacheSize) {
    ...
    public void optimize() {
        try {
            ...
            for (Entry<ArrayPrimitiveWritable, byte[]> entry : this.entrySet()) {
    ...
    @Override
    public byte[] put(ArrayPrimitiveWritable key, byte[] value) {
        if (key == null)
            return null;
    ...
    @Override
    public void putAll(Map<? extends ArrayPrimitiveWritable, ? extends byte[]> m) {
        try {
            WriteBatch batch = db.createWriteBatch();
            int counter = 0;
            for (Map.Entry<? extends ArrayPrimitiveWritable, ? extends byte[]> e : m
                    .entrySet()) {
    ...
    @Override
    public Set<java.util.Map.Entry<ArrayPrimitiveWritable, byte[]>> entrySet() {
        return new MapEntrySet<ArrayPrimitiveWritable, byte[]>(this, this.keyDeSerFunc,
                this.valDeSerFunc);
    }
