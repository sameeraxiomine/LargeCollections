...
public class IntWritableArrayPrimitiveWritableMap extends LargeCollection implements   Map<IntWritable,ArrayPrimitiveWritable>, Serializable{
    ...
    private transient Function<byte[], IntWritable> keyDeSerFunc     = new WritableSerDes.IntWritableDeSerFunction();
    ...
    public IntWritableArrayPrimitiveWritableMap() {
        super();
    }
    
    public IntWritableArrayPrimitiveWritableMap(String dbName) {
        super(dbName);
    }
    
    public IntWritableArrayPrimitiveWritableMap(String dbPath, String dbName) {
        super(dbPath, dbName);
    }
    
    public IntWritableArrayPrimitiveWritableMap(String dbPath, String dbName, int cacheSize) {
    ...
    public void optimize() {
        try {
            ...
            for (Entry<IntWritable, ArrayPrimitiveWritable> entry : this.entrySet()) {
    ...
    @Override
    public ArrayPrimitiveWritable put(IntWritable key, ArrayPrimitiveWritable value) {
        if (key == null)
            return null;
    ...
    @Override
    public void putAll(Map<? extends IntWritable, ? extends ArrayPrimitiveWritable> m) {
        try {
            WriteBatch batch = db.createWriteBatch();
            int counter = 0;
            for (Map.Entry<? extends IntWritable, ? extends ArrayPrimitiveWritable> e : m
                    .entrySet()) {
    ...
    @Override
    public Set<java.util.Map.Entry<IntWritable, ArrayPrimitiveWritable>> entrySet() {
        return new MapEntrySet<IntWritable, ArrayPrimitiveWritable>(this, this.keyDeSerFunc,
                this.valDeSerFunc);
    }
