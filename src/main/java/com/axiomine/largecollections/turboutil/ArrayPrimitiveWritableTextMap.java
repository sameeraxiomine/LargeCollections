...
public class ArrayPrimitiveWritableTextMap extends LargeCollection implements   Map<ArrayPrimitiveWritable,Text>, Serializable{
    ...
    private transient TurboSerializer<Text> valSerFunc  = new WritableSerDes.TextSerFunction();    
    ...
    private transient TurboDeSerializer<Text> valDeSerFunc     = new WritableSerDes.TextDeSerFunction();
    
    public ArrayPrimitiveWritableTextMap() {
        super();
    }
    
    public ArrayPrimitiveWritableTextMap(String dbName) {
        super(dbName);
    }
    
    public ArrayPrimitiveWritableTextMap(String dbPath, String dbName) {
        super(dbPath, dbName);
    }
    
    public ArrayPrimitiveWritableTextMap(String dbPath, String dbName, int cacheSize) {
    ...
    public void optimize() {
        try {
            ...
            for (Entry<ArrayPrimitiveWritable, Text> entry : this.entrySet()) {
    ...
    @Override
    public Text put(ArrayPrimitiveWritable key, Text value) {
        if (key == null)
            return null;
    ...
    @Override
    public void putAll(Map<? extends ArrayPrimitiveWritable, ? extends Text> m) {
        try {
            WriteBatch batch = db.createWriteBatch();
            int counter = 0;
            for (Map.Entry<? extends ArrayPrimitiveWritable, ? extends Text> e : m
                    .entrySet()) {
    ...
    @Override
    public Set<java.util.Map.Entry<ArrayPrimitiveWritable, Text>> entrySet() {
        return new MapEntrySet<ArrayPrimitiveWritable, Text>(this, this.keyDeSerFunc,
                this.valDeSerFunc);
    }
    ...
    private void readObject(java.io.ObjectInputStream in) throws IOException,
        ...
        valSerFunc  = new WritableSerDes.TextSerFunction();    
