...
public class TextArrayPrimitiveWritableMap extends LargeCollection implements   Map<Text,ArrayPrimitiveWritable>, Serializable{
    ...
    private transient Function<byte[], Text> keyDeSerFunc     = new WritableSerDes.TextDeSerFunction();
    ...
    public TextArrayPrimitiveWritableMap() {
        super();
    }
    
    public TextArrayPrimitiveWritableMap(String dbName) {
        super(dbName);
    }
    
    public TextArrayPrimitiveWritableMap(String dbPath, String dbName) {
        super(dbPath, dbName);
    }
    
    public TextArrayPrimitiveWritableMap(String dbPath, String dbName, int cacheSize) {
    ...
    public void optimize() {
        try {
            ...
            for (Entry<Text, ArrayPrimitiveWritable> entry : this.entrySet()) {
    ...
    @Override
    public ArrayPrimitiveWritable put(Text key, ArrayPrimitiveWritable value) {
        if (key == null)
            return null;
    ...
    @Override
    public void putAll(Map<? extends Text, ? extends ArrayPrimitiveWritable> m) {
        try {
            WriteBatch batch = db.createWriteBatch();
            int counter = 0;
            for (Map.Entry<? extends Text, ? extends ArrayPrimitiveWritable> e : m
                    .entrySet()) {
    ...
    @Override
    public Set<java.util.Map.Entry<Text, ArrayPrimitiveWritable>> entrySet() {
        return new MapEntrySet<Text, ArrayPrimitiveWritable>(this, this.keyDeSerFunc,
                this.valDeSerFunc);
    }
