...
public class IntWritableCharacterMap extends LargeCollection implements   Map<IntWritable,Character>, Serializable{
    ...
    public IntWritableCharacterMap() {
        super();
    }
    
    public IntWritableCharacterMap(String dbName) {
        super(dbName);
    }
    
    public IntWritableCharacterMap(String dbPath, String dbName) {
        super(dbPath, dbName);
    }
    
    public IntWritableCharacterMap(String dbPath, String dbName, int cacheSize) {
    ...
    public void optimize() {
        try {
            ...
            for (Entry<IntWritable, Character> entry : this.entrySet()) {
    ...
    @Override
    public Character put(IntWritable key, Character value) {
        if (key == null)
            return null;
    ...
    @Override
    public void putAll(Map<? extends IntWritable, ? extends Character> m) {
        try {
            WriteBatch batch = db.createWriteBatch();
            int counter = 0;
            for (Map.Entry<? extends IntWritable, ? extends Character> e : m
                    .entrySet()) {
    ...
    @Override
    public Set<java.util.Map.Entry<IntWritable, Character>> entrySet() {
        return new MapEntrySet<IntWritable, Character>(this, this.keyDeSerFunc,
                this.valDeSerFunc);
    }
